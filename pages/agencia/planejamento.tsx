...
export default function PlanejamentoAgencia() {
  ...
  const carregarPilotos = async () => {
    try {
      ...
      // Buscar membro agência associado ao usuário (primeiro por user_id, depois por email como fallback)
      let membro = null;
      let membroError = null;

      console.log('[AgenciaPlanejamento] Carregando pilotos para usuário:', { userId: user?.id, email: user?.email });
      ...
      if (membroPorId && !errorPorId) {
        membro = membroPorId;
        console.log('[AgenciaPlanejamento] Membro encontrado por user_id:', membro.id);
      } else {
        console.log('[AgenciaPlanejamento] Membro não encontrado por user_id, tentando por email:', user?.email);
        ...
        if (membroPorEmail && !errorPorEmail) {
          ...
          console.log('[AgenciaPlanejamento] Membro encontrado por email. User_id atual:', membroPorEmail.user_id);
          
          // Se encontrou por email mas user_id está null, tentar atualizar
          if (!membroPorEmail.user_id && user?.id) {
            console.log('[AgenciaPlanejamento] Tentando vincular user_id ao membro...');
            await supabase
              .from('membros')
              .update({ user_id: user.id })
              .eq('id', membroPorEmail.id);
            console.log('[AgenciaPlanejamento] Vinculação user_id tentada');
          }
        } else {
      ...
      if (membroError || !membro) {
        console.error('[AgenciaPlanejamento] Erro ao buscar membro:', { 
          errorPorId, 
      ...
      console.log('[AgenciaPlanejamento] Membro encontrado, carregando pilotos:', membro.id);
  ...
  const carregarBaloesPiloto = async () => {
    try {
      ...
      console.log('[AgenciaPlanejamento] Carregando balões para usuário:', { userId: user?.id, email: user?.email });
      ...
      console.log('[AgenciaPlanejamento] Membro encontrado, carregando balões:', membro.id);
  ...
  const handleSubmit = async () => {
    console.log('[DEBUG-AGENCIA] Iniciando criação de voo');
    console.log('[DEBUG-AGENCIA] User object:', {
      id: (user as any)?.id,
      users_table_id: (user as any)?.users_table_id,
      email: (user as any)?.email,
      role: (user as any)?.role
    });
    
    if (!user) {
      console.error('[DEBUG-AGENCIA] Usuário não autenticado');
      toast({
        title: "Erro",
    ...
    try {
      ...
      console.log('[DEBUG-AGENCIA] Buscando dados do membro agência...');
      // Buscar dados do membro (agência)
      const { data: membro, error: membroError } = await supabase
        .from('membros')
        .select('*')
        .eq('user_id', (user as any)?.users_table_id)
        .single();

      console.log('[DEBUG-AGENCIA] Resultado busca membro:', {
        membro: membro,
        error: membroError,
        user_id_usado: (user as any)?.users_table_id
      });

      if (membroError || !membro) {
        console.error('[DEBUG-AGENCIA] Erro ao buscar membro:', membroError);
        toast({
          title: "Erro ao carregar dados",
          description: "Agência não encontrada no sistema. Entre em contato com o administrador.",
          variant: "destructive"
      ...
      console.log('[DEBUG-AGENCIA] Membro encontrado:', {
        id: membro.id,
        nome: membro.nome,
      ...
      // Verificar vínculo com o piloto selecionado
      console.log('[DEBUG-AGENCIA] Verificando vínculo com piloto:', formData.piloto_id);
      const { data: vinculo, error: vinculoError } = await supabase
        .from('vinculos_agencia_piloto')
        .select('*')
        .eq('agencia_id', membro.id)
        .eq('piloto_id', formData.piloto_id)
        .eq('status', 'aceito')
        .single();

      console.log('[DEBUG-AGENCIA] Resultado verificação vínculo:', {
        vinculo: vinculo,
        error: vinculoError,
        agencia_id: membro.id,
        piloto_id: formData.piloto_id
      ...
      if (vinculoError || !vinculo) {
        console.error('[DEBUG-AGENCIA] Vínculo não encontrado ou inválido:', vinculoError);
        toast({
          title: "Erro",
          description: "Vínculo com o piloto não encontrado ou não aceito",
          variant: "destructive"
        });
        return;
      }

      console.log('[DEBUG-AGENCIA] Preparando dados do voo:', membro.id);
      ...
      console.log('[DEBUG-AGENCIA] Dados do voo a ser criado:', {
         ...
         totalAdultos,
         totalCriancas,
         created_by: (user as any)?.users_table_id
       });
       ...
       const vooPayload = {
         ...
         baloes_data: baloesSelecionados.map(bs => ({
       ...
       console.log('[DEBUG-AGENCIA] Payload para API:', vooPayload);

       // Obter token de autenticação
       const { data: { session } } = await supabase.auth.getSession();
       if (!session?.access_token) {
         console.error('[DEBUG-AGENCIA] Token de acesso não encontrado');
         toast({
           title: "Erro",
       ...
       console.log('[DEBUG-AGENCIA] Chamando API de criação de voo...');
       ...
       console.log('[DEBUG-AGENCIA] Resposta da API:', {
       ...
       if (!response.ok) {
         console.error('[DEBUG-AGENCIA] Erro detalhado da API:', {
           status: response.status,
           responseData,
           isRLSError: responseData.rls_error,
           context: responseData.context
         });
         
         // Se for erro de RLS, logar informações específicas
         if (responseData.rls_error) {
           console.error('[DEBUG-AGENCIA] Erro RLS detectado:', {
             code: responseData.code,
             message: responseData.message,
             details: responseData.details,
             hint: responseData.hint,
             user_context: responseData.context
           });
         }
         ...
         if (responseData.code === '23505') {
           ...
           toast({
             title: "Erro",
             description: responseData.error || "Erro ao criar voo",
             variant: "destructive"
           });
         }
         return;
       ...
       const voo = responseData.voo;
       console.log('[DEBUG-AGENCIA] Voo criado com sucesso via API:', {
         voo_id: voo.id,
         total_baloes: baloesSelecionados.length,
         total_passageiros: totalAdultos + totalCriancas
       });
      ...
      console.error('[DEBUG-AGENCIA] Erro inesperado:', error);
      toast({
        title: "Erro",
