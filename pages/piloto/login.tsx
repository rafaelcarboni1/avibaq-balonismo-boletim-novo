...
export default function PilotoLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Verifica se o e-mail está na tabela users com role piloto
    ...
    if (!userData || userData.role !== "piloto") {
      setError("Acesso negado: esta área é restrita para pilotos.");
    ...
    if (userData.primeira_senha) {
      router.push("/piloto/trocar-senha");
    } else {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(resetEmail, {
      redirectTo: `${window.location.origin}/piloto/nova-senha`,
    });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <Card className="w-full max-w-md mx-auto">
        <CardHeader className="text-center">
          ...
          <h2 className="text-xl font-semibold">Login Piloto</h2>
        ...
        <CardContent>
          ...
          <div className="mt-6 pt-6 border-t text-center">
            <p className="text-sm text-gray-600">
              É uma agência? 
              <Link href="/agencia/login" className="text-blue-600 hover:underline ml-1">
                Clique aqui
              </Link>
            </p>
