...
export default function PlanejamentoVoo() {
  ...
  const carregarBaloes = async () => {
    try {
      ...
      console.error('[DEBUG-PILOTO] Erro inesperado:', {
        error,
  ...
  const handleSubmit = async () => {
    console.log('[DEBUG-PILOTO] Iniciando criação de voo');
    console.log('[DEBUG-PILOTO] User object:', {
      id: (user as any)?.id,
    ...
    try {
      ...
      console.log('[DEBUG-PILOTO] Validação inicial passou, buscando dados do membro piloto...');

      // Buscar membro associado ao usuário (primeiro por user_id, depois por email como fallback)
      let membro = null;
      let membroError = null;

      console.log('[DEBUG-PILOTO] Buscando membro para usuário:', { 
        userId: user?.id, 
        users_table_id: (user as any)?.users_table_id,
        email: user?.email 
      });
      ...
      if (membroPorId && !errorPorId) {
        ...
        console.log('[DEBUG-PILOTO] Tentando buscar por email:', user?.email);
        ...
        if (membroPorEmail && !errorPorEmail) {
          ...
          // Se encontrou por email mas user_id está null, tentar atualizar
          if (!membroPorEmail.user_id && user?.id) {
            console.log('[DEBUG-PILOTO] Tentando vincular user_id ao membro...');
            const { error: updateError } = await supabase
              .from('membros')
              .update({ user_id: user.id })
              .eq('id', membroPorEmail.id);
            console.log('[DEBUG-PILOTO] Resultado vinculação user_id:', { error: updateError });
          }
        } else {
          membroError = errorPorEmail || errorPorId;
        }
      ...
      if (membroError || !membro) {
        console.error('[DEBUG-PILOTO] Erro ao buscar membro:', { 
          errorPorId, 
          errorPorEmail: membroError, 
          userEmail: user?.email, 
          userId: user?.id,
      ...
      console.log('[DEBUG-PILOTO] Dados do voo a serem criados:', {
      ...
      console.log('[DEBUG-PILOTO] Payload para API:', vooPayload);

      // Obter token de autenticação
      const { data: { session } } = await supabase.auth.getSession();
      if (!session?.access_token) {
        console.error('[DEBUG-PILOTO] Token de acesso não encontrado');
        toast({
          title: "Erro",
      ...
      console.log('[DEBUG-PILOTO] Chamando API de criação de voo...');
      ...
      console.log('[DEBUG-PILOTO] Resposta da API:', {
      ...
      if (!response.ok) {
        console.error('[DEBUG-PILOTO] Erro detalhado da API:', {
          status: response.status,
          responseData,
        ...
        if (responseData.rls_error) {
          console.error('[DEBUG-PILOTO] Erro RLS detectado:', {
      ...
      console.log('[DEBUG-PILOTO] Voo criado com sucesso via API:', voo.id);
      toast({
        title: "Sucesso",
