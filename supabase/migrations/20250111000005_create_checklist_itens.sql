...
BEGIN
  ...
  IF NEW.marcado = false AND (NEW.motivo_nao_marcado IS NULL OR trim(NEW.motivo_nao_marcado) = '' OR trim(NEW.motivo_nao_marcado) = 'Aguardando preenchimento') THEN
...
SELECT 
  ...
  -- Percentual geral
  ROUND(
    (COUNT(CASE WHEN ci.marcado = true OR (ci.motivo_nao_marcado IS NOT NULL AND ci.motivo_nao_marcado != 'Aguardando preenchimento') THEN 1 END) * 100.0) / 
    NULLIF(COUNT(*), 0), 
    2
